...
class Slave(threading.Thread):
    ...
    def __init__(self):
        ...
        self.__localMachine = self.__machineInitial( my_file, "local", _debug )
        logger.debug( "Local machine is confirmed." )

        # Initializa master machine 
        logger.debug( "\nMaster machine check..." )
        my_file = "./MasterMachineFile.plk"
        self.__masterMachine = self.__machineInitial( my_file, "master", _debug )
    ...
    def __machineInitial( self, _path, _type, _debug ):
        my_file = Path( _path )
        if my_file.is_file():
        ...
        if isinstance( _obj, SystemCall ):
            ...
            if content == "start transmit":
                ...
                if self.__st == None or not self.__st.is_alive():
                    ...
                    pass
                self.__clock = float( content_2 )
                self.__st.updateClock( content_2 ) 

            elif content == "kill process":
                if self.__st == None or not self.__st.is_alive():
                    self.__startTransmit()
                else:
                    pass
                ...
                try :
                    psutil.Process( int( content_2 ) ).kill()
    ...
    def __startTransmit(self):
        defaultRequest = 4
        self.__st = SendingStatus( self.__msgCenter, self.__ec, self.__clock, defaultRequest ) 
        self.__st.start()
