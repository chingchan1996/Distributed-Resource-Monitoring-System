...
class Decoder(Coder):
    ...
    def decodeMessage( self, _newMsg:Message ):
        ...
        if type == "sys":
            ...
            data_p = content.pop()
            data_os = content.pop()
            rtn = [ Status_Software( sender, data_os.pop(), data_os[0] ), Status_Processes( sender, data_p.pop(), data_p[0] ) ]
        elif type == "status_all":
            # content = [ data_h, data2_os, data3_p ]
            data_p = content.pop()
            data_os = content.pop()
            data_h = content.pop()
            rtn = [ Status_Hardware( sender, data_h.pop(), data_h[0] ), Status_Software( sender, data_os.pop(), data_os[0] ), Status_Processes( sender, data_p.pop(), data_p[0] ) ]
        elif type == "status_p":
